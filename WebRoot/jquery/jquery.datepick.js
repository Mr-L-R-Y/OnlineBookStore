...
(function ($) {
  ...
  function Datepick() {
    ...
    this.dpDiv = $(
      '<div id="' + this._mainDivId + '" style="display: none;"></div>'
    );
  }
  ...
  $.extend(Datepick.prototype, {
    ...
    _connectDatepick: function (target, inst) {
      ...
      if (showOn == "button" || showOn == "both") {
        ...
        trigger.click(function () {
          ...
          else $.datepick._showDatepick(target);
          return false;
        });
      }
      input
    ...
    _inlineDatepick: function (target, inst) {
      ...
      inst.dpDiv.width(
        this._getNumberOfMonths(inst)[1] *
      ...
      if ($.datepick._datepickerShowing)
        switch (event.keyCode) {
          ...
          case 13:
            var sel = $(
              "td." +
                $.datepick._dayOverClass +
                ", td." +
                $.datepick._currentClass,
              inst.dpDiv
            );
            if (sel[0])
              $.datepick._selectDay(
      ...
      for (var iFormat = 0; iFormat < dateFormat.length; iFormat++)
        if (literal)
          if (dateFormat.charAt(iFormat) == "'" && !lookAhead("'"))
            literal = false;
          else chars += dateFormat.charAt(iFormat);
        else
    ...
    _showDatepick: function (input) {
      ...
      if (
        $.datepick._isDisabledDatepick(input) ||
      ...
      // fix width for dynamic number of date pickers
      inst.dpDiv.width(
        $.datepick._getNumberOfMonths(inst)[1] *
      ...
      if (!inst.inline) {
        ...
        if ($.effects && $.effects[showAnim])
          inst.dpDiv.show(
            showAnim,
            $.datepick._get(inst, "showOptions"),
            duration,
            postProcess
          );
    ...
    _getExtras: function (elem) {
      ...
      return [
        parseInt(convert(elem.css("border-left-width"))) +
    ...
    _checkOffset: function (inst, offset, isFixed) {
      ...
      var browserHeight =
        ...
        (document.documentElement
          ? document.documentElement.clientHeight
          : document.body.clientHeight);
      if (browserWidth == 0) return offset;
      var scrollX =
        document.documentElement.scrollLeft || document.body.scrollLeft;
      var scrollY =
        document.documentElement.scrollTop || document.body.scrollTop;
      ...
      if (offset.top + inst.dpDiv.height() - scrollY > browserHeight)
        offset.top = Math.max(
          ...
          pos[1] -
            (isFixed ? scrollY : 0) -
            (this._inDialog ? 0 : inst.dpDiv.height()) -
            (isFixed && $.browser.opera
    ...
    _doHover: function (id, year, month, td) {
      ...
      onHover.apply(inst.input ? inst.input[0] : null, [
        year ? new Date(year, month, $(td).text()) : null,
        inst,
      ]);
    },
    ...
    _selectDay: function (id, year, month, td) {
      ...
      if (inst.stayOpen) {
        ...
        inst.rangeStart = null;
        if (inst.inline) this._updateDatepick(inst);
      }
    },
    ...
    parseDate: function (format, value, settings) {
      ...
      settings = settings || {};
      var shortYearCutoff =
        settings.shortYearCutoff || this._defaults.shortYearCutoff;
      var dayNamesShort =
        settings.dayNamesShort || this._defaults.dayNamesShort;
      var dayNames = settings.dayNames || this._defaults.dayNames;
      var monthNamesShort =
        settings.monthNamesShort || this._defaults.monthNamesShort;
    ...
    _setDateFromField: function (inst) {
      ...
      var dates = inst.input
        ? inst.input.val().split(this._get(inst, "rangeSeparator"))
        : null;
      inst.endDay = inst.endMonth = inst.endYear = null;
    ...
    _generateHTML: function (inst) {
      ...
      if (maxDate) {
        // don't show past maximum unless also restricted by minimum
      ...
      prevBigText = !navigationAsDateFormat
        ? prevBigText
        : this.formatDate(
            prevBigText,
            this._daylightSavingAdjust(
              new Date(drawYear, drawMonth - stepBigMonths, 1)
            ),
            this._getFormatConfig(inst)
          );
      var prev =
        '<div class="datepick-prev">' +
        (this._canAdjustMonth(inst, -1, drawYear, drawMonth)
          ? (showBigPrevNext
              ? "<a onclick=\"jQuery.datepick._adjustDate('#" +
                inst.id +
                "', -" +
                stepBigMonths +
                ", 'M');\"" +
          ...
          : hideIfNoPrevNext
          ? "&#xa0;"
          : (showBigPrevNext ? "<label>" + prevBigText + "</label>" : "") +
            "<label>" +
            prevText +
            "</label>") +
        "</div>";
      ...
      nextBigText = !navigationAsDateFormat
        ? nextBigText
        : this.formatDate(
            nextBigText,
            this._daylightSavingAdjust(
      ...
      var html =
        ...
        (this._isInRange(inst, gotoDate)
          ? '<div class="datepick-current">' +
            "<a onclick=\"jQuery.datepick._gotoToday('#" +
            inst.id +
            "');\"" +
            this._addStatus(
              showStatus,
              inst.id,
              this._get(inst, "currentStatus"),
              initStatus
            ) +
            ">" +
            currentText +
      ...
      for (var row = 0; row < numMonths[0]; row++)
        for (var col = 0; col < numMonths[1]; col++) {
          ...
          for (var dow = 0; dow < 7; dow++) {
            ...
            var dayStatus =
              status.indexOf("DD") > -1
                ? status.replace(/DD/, dayNames[day])
            ...
            html +=
              ...
              (!changeFirstDay
                ...
                : "<a onclick=\"jQuery.datepick._changeFirstDay('#" +
                  inst.id +
                  "', " +
                  day +
                  ');"' +
      ...
      html +=
        (showStatus
          ? '<div style="clear: both;"></div><div id="datepick-status-' +
            inst.id +
            '" class="datepick-status">' +
            initStatus +
            "</div>"
          : "") +
        (!closeAtTop && !inst.inline ? controls : "") +
    ...
    _generateMonthYearHeader: function (
      ...
      if (secondary || !changeMonth)
        ...
        monthHtml +=
          '<select class="datepick-new-month" ' +
          "onchange=\"jQuery.datepick._selectMonthYear('#" +
          inst.id +
          "', this, 'M');\" " +
          "onclick=\"jQuery.datepick._clickMonthYear('#" +
          inst.id +
          "');\"" +
          this._addStatus(
            showStatus,
            inst.id,
            this._get(inst, "monthStatus"),
            initStatus
          ) +
          ">";
      ...
      // year selection
      if (secondary || !changeYear) html += "<span>" + drawYear + "</span>";
      else {
        ...
        html +=
          '<select class="datepick-new-year" ' +
          "onchange=\"jQuery.datepick._selectMonthYear('#" +
          inst.id +
          "', this, 'Y');\" " +
          "onclick=\"jQuery.datepick._clickMonthYear('#" +
    ...
    _addStatus: function (showStatus, id, text, initStatus) {
      return showStatus
        ? " onmouseover=\"jQuery('#datepick-status-" +
            id +
            "').html('" +
            (text || initStatus) +
            "');\" " +
            "onmouseout=\"jQuery('#datepick-status-" +
            id +
            "').html('" +
            initStatus +
            "');\""
  ...
  $.fn.datepick = function (options) {
    ...
    if (
      ...
      return $.datepick["_" + options + "Datepick"].apply(
        $.datepick,
        [this[0]].concat(otherArgs)
      );
    return this.each(function () {
      typeof options == "string"
        ? $.datepick["_" + options + "Datepick"].apply(
            $.datepick,
